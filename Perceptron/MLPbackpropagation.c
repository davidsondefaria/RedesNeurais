...
//Como funciona essa estrutura? pq tem theNet no final?
...
//variaveis static mantém seu valor independente da função em que recebem 
...
int main() {
    ...
    for(i = 0; i < 50; i++) {
        ...
        updateNetOutput();
        outputs = getOutputs();
        printf( "%f \t %f \t %f \t %f \t %f \n", sin(tempTotal), outputs[0], cos(tempTotal), outputs[1], tempTotal);
        //fprintf(arq, "%f \t %f \t %f \t %f \t %f \n", sin(tempTotal), outputs[0], cos(tempTotal), outputs[1], tempTotal);
    }
    fclose(arq);
    //getch();			//usado no windows
    //return 0;
